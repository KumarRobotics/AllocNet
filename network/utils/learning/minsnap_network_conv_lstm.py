...
class ConvLSTMMinimalSnapNetwork(nn.Module):
    def __init__(self, seg, max_poly_length, lr=1e-3,
                 ...
                 loss_dim_reduction='mean', w1=1, wt= 5.0, wc=1.0, wp=1.0, seq_len=5):
        super().__init__()
        #init
    ...
    def forward(self, stacked_state, stacked_hpolys):
        ...
        if stop_tokens.shape[1] < self.seq_len:
            # print("shape of stop_tokens is : ", stop_tokens.shape)
            # print("shape of torch.ones(10 - stop_tokens.shape[0]).to(self.device) is : ",  torch.ones(10 - stop_tokens.shape[0]).to(self.device).shape)
            # Pad with 1
            stop_tokens_padding = torch.ones(self.seq_len - stop_tokens.shape[1])
            ...
            # print("shape of stop_tokens is (<10) : ", stop_tokens.shape)
        # else:
        # print("shape of stop_tokens is (>=10) : ", stop_tokens.shape)

        # Check if tf has 10 elements
        if tf.shape[1] < self.seq_len:
            # Pad with 0
            tf_padding = torch.zeros(self.seq_len - tf.shape[1])
    ...
    def forward_batch(self, stacked_state, stacked_hpolys, traj_times):
        ...
        for j in range(combined.shape[0]):
            ...
            for k in range(self.seq_len):
                ...
                token_thresh = 0.42
                if curr_stop_tokens.item() > token_thresh:
                    break

            # Check if stop_tokens has 10 elements
            print("shape of stop_tokens is : ", stop_tokens.shape)
        ...
        for i in range(tfs.shape[0]):
            ...
            qp_traj.update(x_i, hpoly_i, tf, phase=self.phase, traj_times=traj_times_i, seq_len=self.seq_len)
            # res = osqplayer(qp_traj)


            solution, curr_obj1_val, curr_objt_val, curr_objc_val, curr_stop_token_loss = osqplayer.forward4lstm(qp_traj, stop_tokens_i, self.seq_len)
            
            if curr_stop_token_loss < 1.0:
                num_time_segment_accurate += 1
            ...
            if solution != None:
                ...
                if curr_stop_token_loss < 1.0:
                    num_time_segment_accurate_if_has_solution += 1
                ...
                curr_obj_value = curr_obj1_val*self.w1 + curr_objc_val * self.wc + curr_stop_token_loss*self.wp
                ...
                stop_token_loss_values.append(curr_stop_token_loss)
                
            else:

                curr_obj_value = curr_obj1_val*self.w1 + curr_objt_val*self.wt + curr_stop_token_loss*self.wp
