from utils.learning.minsnap_network_conv import ConvMinimalSnapNetwork
...
models_dir = "models/minsnap_new_ds_12_1c1c3l_upl16"
...
def sample_input():
    ...
    print("shape of start_state: ", start_state.shape)
    print("shape of end_state: ", end_state.shape)
    print("start_state: ", start_state)
    print("end_state: ", end_state)
    print("goal: ", goal)
    stacked_state = np.vstack((start_state, end_state))

    print("shape of stacked_state: ", stacked_state.shape)
...
def main():
    ...
    model = ConvMinimalSnapNetwork(seg=seg, max_poly_length=max_hpoly_length, hidden_size=256)
    model.to(device)
    checkpoint = torch.load(model_dir)
    model.load_state_dict(checkpoint["model_state_dict"])
    
    comp_start_time = time.time()
    solution, obj1_val, objt_val, objc_val = model(stacked_state, stacked_hpolys)
